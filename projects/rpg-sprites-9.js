...
window.RPGSprites9 = (function () {
  ...
  /* ── skiny hrdiny: přebarvení palety (koupené v obchodě) ──
     Přepisují jen barevné klíče (zbroj J/j, hledí C/c, ramena G);
     obrys K a zbraň W zůstávají, takže žádný znak nezůstane nedefinovaný. */
  const HERO_SKINS = {
  ...
  let activeSkin = null;                       // klíč do HERO_SKINS nebo null = výchozí
  ...
  function drawHeroOn(c2, x, y, scale, frame, flipX) {
    ...
    for (let r = 0; r < grid.length; r++) {
      ...
      for (let c = 0; c < row.length; c++) {
        const ch = row[c];
        if (ch === '.') continue;
        c2.fillStyle = pal[ch] || COMMON[ch] || '#f0f';
        const px = flipX ? x + (row.length - 1 - c) * scale : x + c * scale;
