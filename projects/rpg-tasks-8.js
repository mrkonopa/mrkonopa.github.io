...
window.RPG_TASK_EXTRA_8 = {
 ...
 '2-2': () => [   // Pythagoras — přepona
  ...
  (()=>{const a=ri(3,9);let b=ri(3,12);if(b===a)b=b<12?b+1:b-1; /* strany různé — jinak z „obdélníku" vyjde čtverec */ const c=Math.sqrt(a*a+b*b);return{text:`Pravoúhlý trojúhelník: a = ${a}, b = ${b}.\nPřepona c? (2 des. místa)`,ans:r2(c),hints:[`c² = ${a*a} + ${b*b} = ${a*a+b*b}.`,`Odmocni ${a*a+b*b}.`],skill:'geo'};})(),
  (()=>{const a=ri(4,10);let b=ri(3,8);if(b===a)b=b<8?b+1:b-1; /* strany různé — jinak z „obdélníku" vyjde čtverec */ const c=Math.sqrt(a*a+b*b);return{text:`Žebřík: pata ${b} m od zdi, dosáhne do výšky ${a} m.\nDélka žebříku? (2 des. místa)`,ans:r2(c),hints:[`Žebřík = přepona.`,`√(${a}² + ${b}²)`],skill:'geo'};})(),
  (()=>{const a=ri(6,12);let b=ri(4,10);if(b===a)b=b<10?b+1:b-1; /* strany různé — jinak z „obdélníku" vyjde čtverec */ const c=Math.sqrt(a*a+b*b);return{text:`Obdélník ${a} × ${b} cm.\nÚhlopříčka? (2 des. místa)`,ans:r2(c),hints:[`Úhlopříčka = přepona trojúhelníku.`,`√(${a}² + ${b}²)`],skill:'geo'};})(),
  ...
  (()=>{const a=ri(4,8);let b=ri(3,7);if(b===a)b=b<7?b+1:b-1; /* strany různé — jinak z „obdélníku" vyjde čtverec */ const c=Math.sqrt(a*a+b*b);return{text:`Střecha: sklon ${a} m výšky, délka ${b} m.\nDélka krokve? (2 des. místa)`,ans:r2(c),hints:[`Krokev = přepona.`,`√(${a}²+${b}²)`],skill:'geo'};})(),
  (()=>{const uc=ri(2,6);let a=ri(5,9);if(a===uc)a=a<9?a+1:a-1; /* strany různé — jinak z „obdélníku" vyjde čtverec */ let b=ri(4,8);if(b===a)b=b<8?b+1:b-1; /* strany různé — jinak z „obdélníku" vyjde čtverec */ const c=Math.sqrt(a*a+b*b);return{text:`V patře akademie ${skl(uc,'je','jsou','je')} ${uc} ${skl(uc,'učebna','učebny','učeben')}, každá obdélníková ${a} × ${b} m.\nJak dlouhá je úhlopříčka jedné učebny? (2 des. místa)`,ans:r2(c),hints:[`Úhlopříčka = přepona pravoúhlého trojúhelníku.`,`√(${a}² + ${b}²)`],skill:'geo'};})(),
 ...
 '3-3': () => [   // Slovní úlohy
  ...
  (()=>{const a=ri(5,12);let b=ri(3,10);if(b===a)b=b<10?b+1:b-1; /* strany různé — jinak z „obdélníku" vyjde čtverec */ return{text:`Obvod obdélníku je ${2*(a+b)} cm, jedna strana ${a} cm.\nDruhá strana? (cm)`,ans:String(b),hints:[`o = 2(a+b), b = o/2 − a.`,`${a+b} − ${a}`],skill:'anal'};})(),
 ...
 '7-2': () => [   // Přijímačkový trénink (CERMAT-style)
  ...
  (()=>{const a=ri(30,60);let b=ri(20,50);if(b===a)b=b<50?b+1:b-1; /* strany různé — jinak z „obdélníku" vyjde čtverec */ const c=Math.sqrt(a*a+b*b);return{text:`Obdélníkové hřiště ${a} × ${b} m.\nDélka úhlopříčky? (2 des. místa)`,ans:r2(c),hints:[`√(${a}² + ${b}²).`],skill:'geo'};})(),
