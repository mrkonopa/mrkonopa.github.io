/* ════════════════════════════════════════════════════════════════════
   RPG Matematika 4 — pixel-art bojová scéna (canvas engine)
   Téma: Pirátská plavba 🏴‍☠️
   ────────────────────────────────────────────────────────────────────
   API (window.RPGSprites4):
   ...
   ════════════════════════════════════════════════════════════════════ */
window.RPGSprites4 = (function () {
  'use strict';

  /* ── palety ── */
  // Hrdina — mladý pirát v námořnické vestě
  const PAL_HERO = {
    K:'#0a0c12', J:'#22417a', j:'#122747', H:'#23252f',
    C:'#f3c89a', c:'#c98a5a', G:'#c33327', g:'#7e160f',
    Y:'#e8c24a', B:'#5a3a1c', m:'#aa5f4a', S:'#c9ccd6',
    W:'#e8ecf5'
  };
  const HERO_SKINS = {
  ...
  /* ── pirát s třírohým kloboukem 🏴‍☠️ — kouká doprava, kord/pistole (18×24) ── */
  const HERO_IDLE = [[
    '......KKKK........',
    '.....KHHHHK.......',
    '...KKHHHHHHKK.....',
  ...
  // mořské pozadí (obloha + moře + vlny + slunce, per oblast)
  const G4_SEAS = {
  ...
  function drawBackdrop(now) {
    ...
    const sea = G4_SEAS[curArea] || G4_SEAS[1];
    ...
    ctx.globalAlpha = 0.85; pxDisc(scx, scy, sr, 3, sea.sun);
    ctx.globalAlpha = 0.55; pxDisc(scx - 6, scy - 6, Math.round(sr * 0.55), 3, sea.sunH);
    ctx.globalAlpha = 1;
    // mraky
    if (sea.cloud) {
      ctx.globalAlpha = 0.40;
      pxDisc(Math.round(W * 0.18), 30, 22, 4, '#e8f4ff');
      pxDisc(Math.round(W * 0.30), 24, 15, 4, '#ffffff');
      ctx.globalAlpha = 1;
    }
    // moře
    ...
    if (animOK) {
      ...
      for (let i = 0; i < 4; i++) {
        ctx.fillStyle = sea.wave;
        const wx = ((woff + i * 0.25) % 1) * W;
        const wy = skyH + 4 + i * 10;
        ctx.fillRect(wx, wy, 38, 3);
        ctx.fillRect((wx + W * 0.5) % W, wy + 2, 28, 2);
      }
      ctx.globalAlpha = 1;
    }
    ...
    for (let i = 0; i < 5; i++) {
      const wx2 = rnd() * W;
      ctx.fillRect(wx2, H - 28 - Math.round(rnd() * 20), 22 + Math.round(rnd() * 20), 2);
    }
    ctx.globalAlpha = 1;
  }
