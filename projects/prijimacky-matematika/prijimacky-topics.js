...
(function () {
  ...
  const TOPICS = [
    ...
    { id: 'vyrazy-promenna', name: 'Výrazy s proměnnou', oblast: 'Číslo a proměnná', slots: [2, 15] },
  ...
  // Vygeneruj procvičovací položku pro daný okruh (náhodná pozice z jeho slotů).
  function practiceItem(topicId) {
    const topic = TOPICS.find(x => x.id === topicId);
    if (!topic || typeof window.RPG_CERMAT_9 === 'undefined') return null;
    const idx = topic.slots[Math.floor(Math.random() * topic.slots.length)];
    let t;
    try { t = window.RPG_CERMAT_9.genSlot(idx); } catch (e) { return null; }
    return taskToItem(t);
  }
