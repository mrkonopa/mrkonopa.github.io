...
(function () {
  ...
  function gen_1_2() {
    ...
    const T = [
      ...
      () => { const [a, b, c] = tri(); return { text: `Které z čísel ${a}, ${b}, ${c} je největší?`, ans: Math.max(a, b, c), mc_opts: [a, b, c], h1: `Porovnej po dvojicích, začni stovkami.`, h2: `= ${Math.max(a, b, c)}`, distractors: [String(Math.min(a, b, c))] }; }, // miskoncepce: vybere nejmenší místo největšího
      () => { const [a, b, c] = tri(); return { text: `Které z čísel ${a}, ${b}, ${c} je nejmenší?`, ans: Math.min(a, b, c), mc_opts: [a, b, c], h1: `Hledej nejmenší stovky.`, h2: `= ${Math.min(a, b, c)}`, distractors: [String(Math.max(a, b, c))] }; }, // miskoncepce: vybere největší místo nejmenšího
  ...
  function gen_4_3() {
    const tasks = [];
    const T = [
      // — beze zbytku (vyjde celek) — širší rozsah podílů (3–15) pro víc rozmanitosti
      () => { const d = ri(2, 7), total = d * ri(3, 15); return { text: pick([`${total} oříšků rozdělíme rovným dílem mezi ${d} ${skl(d, 'veverku', 'veverky', 'veverek')}. Kolik dostane každá?`, `Veverky si spravedlivě rozdělily ${total} oříšků. Je jich ${d}. Kolik oříšků dostala každá?`]), ans: total / d, h1: `${total} : ${d}`, h2: `= ${total / d}` }; },
      () => { const d = ri(2, 8), n = d * ri(3, 15); return { text: `Skřítek rozdělil ${n} hub do ${d} ${skl(d, 'košíku', 'košíků', 'košíků')} stejně. Kolik hub je v jednom košíku?`, ans: n / d, h1: `${n} : ${d}`, h2: `= ${n / d}` }; },
      () => { const cols = ri(3, 14), rows = ri(2, 6); return { text: `${rows * cols} stromků je vysázeno ${rows < 5 ? 've' : 'v'} ${rows} ${skl(rows, 'řadě', 'řadách', 'řadách')} stejně. Kolik stromků je v jedné řadě?`, ans: cols, h1: `${rows * cols} : ${rows}`, h2: `= ${cols}` }; },
      () => { const d = ri(2, 6), q = ri(3, 15); return { text: `Provaz dlouhý ${d * q} m rozstříháme na kusy po ${d} m. Kolik kusů dostaneme?`, ans: q, h1: `${d * q} : ${d}`, h2: `= ${q}` }; },
      () => { const d = ri(2, 5), q = ri(3, 15); return { text: `Medvědice rozdělila ${d * q} medových koláčků rovným dílem mezi ${d} ${skl(d, 'medvídě', 'medvíďata', 'medvíďat')}. Kolik koláčků dostalo každé?`, ans: q, h1: `${d * q} : ${d}`, h2: `= ${q}` }; },
      () => { const cena = ri(3, 9), penize = cena * ri(3, 15); return { text: `Jedna kouzelná fazole stojí ${cena} ${skl(cena, 'zlaťák', 'zlaťáky', 'zlaťáků')}. Kolik fazolí koupíš za ${penize} zlaťáků?`, ans: penize / cena, h1: `${penize} : ${cena}`, h2: `= ${penize / cena}` }; },
      // — se zbytkem — širší rozsah podílů (3–13)
      () => { const d = ri(3, 6), q = ri(3, 13), r = ri(1, d - 1), n = d * q + r; return { text: `Jezevec našel ${n} žaludů a rozděluje je rovným dílem mezi ${d} ${skl(d, 'jezevce', 'jezevce', 'jezevců')}. Kolik žaludů zbyde?`, ans: r, h1: `${d} × ${q} = ${d * q}, zbyde ${n} − ${d * q}.`, h2: `zbyde ${r}` }; },
      () => { const d = ri(3, 6), q = ri(3, 13), r = ri(1, d - 1), n = d * q + r; return { text: `Máš ${n} jablek a dáváš je do pytlíků po ${d}. Kolik pytlíků úplně naplníš?`, ans: q, h1: `${d} × ${q} = ${d * q} se vejde, ${d} × ${q + 1} už ne.`, h2: `= ${q}` }; },
      () => { const d = ri(2, 4), q = ri(3, 13), r = ri(1, d - 1), n = d * q + r; return { text: `Na lavičku se vejdou ${d} ${skl(d, 'skřítek', 'skřítci', 'skřítků')}. Kolik laviček je PLNĚ obsazených, když se posadí ${n} ${skl(n, 'skřítek', 'skřítci', 'skřítků')}?`, ans: q, h1: `${n} : ${d} je ${q}, zbytek ${r}.`, h2: `= ${q}` }; },
      () => { const d = ri(3, 6), q = ri(2, 12), r = ri(1, d - 1), n = d * q + r; return { text: `Víla svazuje kytice po ${d} ${skl(d, 'květině', 'květinách', 'květinách')}. Má ${n} ${skl(n, 'květinu', 'květiny', 'květin')}. Kolik květin jí zbyde mimo kytice?`, ans: r, h1: `Celé kytice: ${q} (${d} × ${q} = ${d * q}). Zbytek: ${n} − ${d * q}.`, h2: `= ${r}` }; },
      () => { const d = ri(2, 6), q = ri(3, 15); return { text: pick([`Skřítek rozvěsí ${d * q} lucerniček rovným dílem na ${d} ${skl(d, 'strom', 'stromy', 'stromů')}. Kolik lucerniček je na jednom stromě?`, `Na ${d} ${skl(d, 'strom', 'stromy', 'stromů')} pověsíme ${d * q} lucerniček stejně. Kolik jich připadne na jeden strom?`]), ans: q, h1: `${d * q} : ${d}`, h2: `= ${q}` }; },
    ];
    for (let i = 0; i < T.length; i++) {
      const t = T[i % T.length]();
      tasks.push({ text: t.text, ans: t.ans, hints: [t.h1, t.h2], skill: 'calc' , mc_opts: t.mc_opts });
