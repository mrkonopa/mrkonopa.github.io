/**
 * Integrace rozšiřující banky úloh do hry rpg-mat-6.html
 * Spusť: node tests/rpg-tasks-6-integration.test.cjs
 */
...
async function run(){
  console.log('\n══════════════════════════════════════════');
  console.log('  rpg-mat-6 — integrace banky úloh');
  console.log('══════════════════════════════════════════\n');
  ...
  try{
    await pg.goto(`${BASE}/projects/rpg-mat-6.html`,{waitUntil:'domcontentloaded'});
    await pg.waitForFunction(()=>typeof AREAS!=='undefined' && typeof launchBattle==='function',{timeout:8000});

    // 1) banka načtená
    const bank=await pg.evaluate(()=>!!window.RPG_TASK_EXTRA_6 && Object.keys(window.RPG_TASK_EXTRA_6).length);
    ok('Banka rpg-tasks-6.js načtená (21 misí)', bank===21, 'misí: '+bank);
    ...
    // distinktní zadání napříč losováním
    ...
    // 4) MC mise: vše numerické
    const mcSafe=await pg.evaluate(()=>{
      const ex=window.RPG_TASK_EXTRA_6;
      const mcIds=['1-1','2-1','3-2'];
    ...
    ok('MC mise mají jen numerické nebo ANO/NE odpovědi', mcSafe);
