/**
 * Integrace rozšiřující banky úloh do hry rpg-mat-7.html
 * Spusť: node tests/rpg-tasks-7-integration.test.cjs
 */
...
async function run(){
  console.log('\n══════════════════════════════════════════');
  console.log('  rpg-mat-7 — integrace banky úloh');
  console.log('══════════════════════════════════════════\n');
  ...
  try{
    ...
    // 1) banka načtená
    const bank=await pg.evaluate(()=>!!window.RPG_TASK_EXTRA_7 && Object.keys(window.RPG_TASK_EXTRA_7).length);
    ok('Banka rpg-tasks-7.js načtená (21 misí)', bank===21, 'misí: '+bank);
    ...
    const sizes=await pg.evaluate(()=>{
      ...
      const ex=window.RPG_TASK_EXTRA_7;
    ...
    const mcSafe=await pg.evaluate(()=>{
      const ex=window.RPG_TASK_EXTRA_7;
      const mcIds=['1-1','2-1','3-1','4-1','5-1','6-1'];
