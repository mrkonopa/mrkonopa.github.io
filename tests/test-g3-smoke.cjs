...
(async () => {
  ...
  console.log('\n━━ rpg-mat-3 smoke test ━━');
  await page.goto(base + '/projects/rpg-mat-3.html', {waitUntil:'load'});
  await sleep(2000);
  ...
  await page.evaluate(()=>{
    if(!window.RPGSprites3) window.RPGSprites3={attach:()=>{},spawn:()=>{},setHeroHp:()=>{},active:()=>false,detach:()=>{},heroAttack:()=>{},bossAttack:()=>{},defeat:()=>{},setSkin:()=>{},drawHeroOn:()=>{}};
    if(!window.RPG_TASK_EXTRA_3) {
