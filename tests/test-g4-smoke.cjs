...
(async () => {
  ...
  console.log('\n━━ rpg-mat-4 smoke test ━━');
  await page.goto(base + '/projects/rpg-mat-4.html', {waitUntil:'load'});
  await sleep(2000);
  ...
  await page.evaluate(()=>{
    if(!window.RPGSprites4) window.RPGSprites4={attach:()=>{},spawn:()=>{},setHeroHp:()=>{},active:()=>false,detach:()=>{},heroAttack:()=>{},bossAttack:()=>{},defeat:()=>{},setSkin:()=>{},drawHeroOn:()=>{}};
    if(!window.RPG_TASK_EXTRA_4) {
