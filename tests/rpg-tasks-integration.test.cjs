/**
 * Integrace rozšiřující banky úloh do hry rpg-mat-9.html
...
async function run(){
  console.log('\n══════════════════════════════════════════');
  console.log('  rpg-mat-9 — integrace banky úloh');
  console.log('══════════════════════════════════════════\n');
  ...
  try{
    ...
    // 1) banka načtená
    const bank=await pg.evaluate(()=>!!window.RPG_TASK_EXTRA_9 && Object.keys(window.RPG_TASK_EXTRA_9).length);
    ok('Banka rpg-tasks-9.js načtená (21 misí)', bank===21, 'misí: '+bank);
    ...
    const sizes=await pg.evaluate(()=>{
      ...
      const ex=window.RPG_TASK_EXTRA_9;
