/**
 * Audit rozšiřující banky úloh rpg-tasks-9.js
 * Spusť: node tests/rpg-tasks-9.audit.cjs
...
global.svgCone=()=>'<svg></svg>';
global.svgSphere=()=>'<svg></svg>';
...
new Function(code)();   // naplní window.RPG_TASK_EXTRA_9
const EX = global.window.RPG_TASK_EXTRA_9;
 ...
 for(let it=0; it<ITER; it++){
  ...
  arr.forEach((t,i)=>{
   ...
   if(MC.has(id)){
    ...
    // mimo MC: ANO/NE nebo numerické nebo zlomek "a/b"
    if(ans==='ANO'||ans==='NE'){ /* ok */ }
    else {
...
console.log('\n══════════════════════════════════════════');
console.log('  Audit rpg-tasks-9.js — rozšiřující banka');
console.log('══════════════════════════════════════════');
